...
const grid=document.getElementById('offersGrid');if(grid)offers.forEach(o=>{const el=document.createElement('article');el.className='offer';el.innerHTML=`<span>${o.tag}</span><h3>${o.city}</h3><h4>${o.hotel}</h4><p>${o.details}</p><strong>${o.price}</strong><a href="en.html#contact">Request details</a>`;grid.appendChild(el)});